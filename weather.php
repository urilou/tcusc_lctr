...
// 警報注意報の取得
  $warning_info = $warning_html->find('table#WarnTableTable',0)->find('tr',10);
  $warning_text = '世田谷区に'.warn_parse($warning_info);
  if ($warning_text == '世田谷区に気象警報があります >>') {
    $warning_text = '';
  }
...
//$weathertmrrw = "明日、".$tmrrw."日の横浜の天気は".$weathertmrrwtext."（最高気温".$weathertmrrwtemp."度、降水確率".$weathertmrrwrain."%）です。（".$weathernowtime."、Foreca）";
  ...
  $weather_now = "いまの世田谷の天気は".$weathernowtext."、気温: ".$weathernowtemp."度（体感".$weathernowfeel."度）、湿度: ".$weathernowhumi."%、".$weathernowwind."、不快指数: ".$weathernowfukai."（".$weathernowfkifl."）です。（".$weathernowtime."、Foreca）";
  echo "<div>".$weather_now."</div>";
  $weather_today = "おはようございます。".$today."日の世田谷の天気は".$weathertodaytext."（最高気温".$weathertodaytemp."度、降水確率".$weathertodayrain."%）です。";
  echo "<div>".$weather_today."</div>";
  $weather_tmrrw = "明日、".$tmrrw."日の世田谷の天気は".$weathertmrrwtext."（最高気温".$weathertmrrwtemp."度、降水確率".$weathertmrrwrain."%）です。（".$weathernowtime."、Foreca）";
...
// 警報注意報ツイート
  if ( !empty ($warning_text) && ($min == 05 || $min == 35)) {
