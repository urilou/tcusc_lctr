<?php
  ...
  $cancel_data_html = file_get_html('http://www.ipc.tcu.ac.jp/~sd/keiji.html');
  $cancel_data_html = mb_convert_encoding($cancel_data_html, 'utf-8', 'sjis-win');
  $cancel_data_html = mb_convert_kana($cancel_data_html,"a","utf-8");
  $cancel_data_html = str_get_html($cancel_data_html);

  $cancel_data = $cancel_data_html->find('body table',0)->innertext;
  ...
  function cancel_today(){
    ...
    for ($run_length = 0; $run_length < $cancel_data_length; $run_length++) {
      $data_item1 = $cancel_data_html->find("td",0+$run_length+($run_length*5))->innertext;
      ...
      if (!strcmp($data_item1,$today)) {
        $data_item2 = $cancel_data_html->find("td",1+$run_length+($run_length*5))->innertext;
        $data_item3 = $cancel_data_html->find("td",2+$run_length+($run_length*5))->innertext;
        $data_item4 = $cancel_data_html->find("td",3+$run_length+($run_length*5))->innertext;
        $data_item5 = $cancel_data_html->find("td",4+$run_length+($run_length*5))->innertext;
        $data_item5 = str_replace('　', '', $data_item5);
        $data_item6 = $cancel_data_html->find("td",5+$run_length+($run_length*5))->innertext;
        $cancel_data[] = '【休講】今日 >> '.$data_item2.'限　'.$data_item3.'（'.$data_item5.'　'.$data_item4.'） '.$data_item6."\n";
      }
    }
    ...
    echo '<div><b>休講</b></div>';
    array_unshift($cancel_data,'今日、'.date('n月j日').'の休講情報（最新の情報は http://bit.ly/tcusclctr で確認してください。）'."\n");
  ...
  function cancel_tomorrow(){
    ...
    for ($run_length = 0; $run_length < $cancel_data_length; $run_length++) {
      ...
      if (!strcmp($data_item1,$tomorrow)) {
        ...
        $data_item5 = str_replace('　', '', $data_item5);
        $data_item6 = $cancel_data_html->find("td",5+$run_length+($run_length*5))->innertext;
        $cancel_data[] = '【休講】'.$data_item1.' '.$data_item2.'限　'.$data_item3.'（'.$data_item5.'　'.$data_item4.'） '.$data_item6."\n";
      }
    }
    ...
    echo '<div><b>休講</b></div>';
    array_unshift($cancel_data,'明日、'.date('n月j日',strtotime('+1 day')).'の休講情報（最新の情報は http://bit.ly/tcusclctr で確認してください。）'."\n");
    foreach ($cancel_data as $key => $value){
      echo $value.'<br>';
    }
    write_array($cancel_routine_file,$cancel_data);
  ...
  function cancel_next($period){
    ...
    for ($run_length = 0; $run_length < $cancel_data_length; $run_length++) {
      ...
      if (!strcmp($data_item1,$today) && strpos($data_item2, $period) !== false) {
        ...
        $data_item5 = str_replace('　', '', $data_item5);
        $data_item6 = $cancel_data_html->find("td",5+$run_length+($run_length*5))->innertext;
        $cancel_data_next[] = '【休講】つぎ >> '.$data_item2.'限　'.$data_item3.'（'.$data_item5.'　'.$data_item4.'） '.$data_item6."\n";
      }
    }
  ...
  function cancel_new(){
    ...
    for ($run_length = 0; $run_length < $cancel_data_length; $run_length++) {
      ...
      $cancel_data_new[] = '[新着]【休講】'.$data_item1.' '.$data_item2.'限　'.$data_item3.'（'.$data_item5.'　'.$data_item4.'） '.$data_item6."\n";
      $cancel_api_v3[] = "{\"day\":\"".$data_item1."\",\"period\":\"".$data_item2."\",\"subject\":\"".$data_item3."\",\"lecturer\":\"".$data_item5."\",\"class\":\"".$data_item4."\",\"note\":\"".$data_item6."\"}";
    }
    ...
    $cancel_api_v3 = "{\"name\":\"都市大SC休講情報(全件)\",\n\"ver\":{\"this\":\"3\",\"last\":\"3\"},\n\"update\":\"".date('Y/n/j H:i:s')."\",\n\"inserted\":\"".count($diff)."\",\n\"data\":[\n".$cancel_api_v3."\n]}";
